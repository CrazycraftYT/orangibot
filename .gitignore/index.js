...
let initialMessage = `**Vous pouvez mettre la réaction si dessous pour recevoir (ou non) les mentions des annonces UHC's.**`;
const roles = ["Mention"];
const reactions = ["✅"];
...
if (roles.length !== reactions.length) throw "Erreur :x:";
 
//Function to generate the role messages, based on your settings
function generateMessages(){
    var messages = [];
    messages.push(initialMessage);
    for (let role of roles) messages.push(`Mettre la réaction si dessous pour obtenir le rôle **"${role}"** !`); //DONT CHANGE THIS
    return messages;
}
